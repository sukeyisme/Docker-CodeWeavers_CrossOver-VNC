...
# 安装ie8浏览器
# prepare
${WINE} Reg.exe add "HKCU\Software\Wine\DllOverrides" /v "iexplore.exe" /t REG_SZ /d "native, builtin" /f
${WINE} Reg.exe add "HKCU\Software\Wine\DllOverrides" /v "itircl" /t REG_SZ /d "native, builtin" /f
${WINE} Reg.exe add "HKCU\Software\Wine\DllOverrides" /v "itss" /t REG_SZ /d "native, builtin" /f
${WINE} Reg.exe add "HKCU\Software\Wine\DllOverrides" /v "jscript" /t REG_SZ /d "native, builtin" /f
${WINE} Reg.exe add "HKCU\Software\Wine\DllOverrides" /v "mlang" /t REG_SZ /d "native, builtin" /f
${WINE} Reg.exe add "HKCU\Software\Wine\DllOverrides" /v "mshtml" /t REG_SZ /d "native, builtin" /f
${WINE} Reg.exe add "HKCU\Software\Wine\DllOverrides" /v "msimtf" /t REG_SZ /d "native, builtin" /f
${WINE} Reg.exe add "HKCU\Software\Wine\DllOverrides" /v "shdoclc" /t REG_SZ /d "native, builtin" /f
${WINE} Reg.exe add "HKCU\Software\Wine\DllOverrides" /v "shdocvw" /t REG_SZ /d "native, builtin" /f
${WINE} Reg.exe add "HKCU\Software\Wine\DllOverrides" /v "urlmon" /t REG_SZ /d "native, builtin" /f
${WINE} Reg.exe add "HKCU\Software\Wine\DllOverrides" /v "inetcpl.cpl" /t REG_SZ /d "native, builtin" /f
...
mv /tmp/exe/winetest.cat "${HOME}/.cxoffice/${BOTTLE}/drive_c/windows/system32/catroot/{f750e6c3-38ee-11d1-85e5-00c04fc295ee}/oem0.cat"

${INSTALLDIR}/bin/wine IE8-WindowsXP-x86-ENU.exe "/quiet /forcerestart"
...
do
    ${INSTALLDIR}/bin/wine regsvr32 /i $i > /dev/null 2>&1
done

${WINE} Reg.exe add "HKCU\Software\Microsoft\Internet Explorer\Main" /v "TabProcGrowth" /t REG_DWORD /d "00000000" /f

${INSTALLDIR}/bin/wine IE8-WindowsXP-KB2936068-x86-ENU.exe '/quiet /forcerestart'
status=$?
case $status in
  0|194) ;;
  *) echo "$W_PACKAGE installation failed"
esac

${INSTALLDIR}/bin/wine --bottle ${BOTTLE} --untrusted --wait-children --no-convert --new-console --dll 'msxml3=n' -- 'msiexec.exe' '/i' '/tmp/msxml3.msi' '/quiet'
