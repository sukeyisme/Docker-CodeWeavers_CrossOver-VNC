...
if [ -f "${HOME}/.cxoffice/${BOTTLE}/drive_c/Program Files/Internet Explorer/iexplore.exe" ]; then
    if grep -q -i "wine placeholder" "${HOME}/.cxoffice/${BOTTLE}/drive_c/Program Files/Internet Explorer/iexplore.exe"; then
        ...
        ${INSTALLDIR}/bin/wine --bottle ${BOTTLE} iexplore -unregserver
    fi
...
for dll in browseui inseng itircl itss jscript msctf mshtml shdoclc shdocvw shlwapi urlmon; do
    test -f "${HOME}/.cxoffice/${BOTTLE}/drive_c/windows/system32"/${dll}.dll && mv "${HOME}/.cxoffice/${BOTTLE}/drive_c/windows/system32"/$dll.dll "${HOME}/.cxoffice/${BOTTLE}/drive_c/windows/system32"/${dll}.dll.bak
done
...
${INSTALLDIR}/bin/wine --bottle ${BOTTLE} --untrusted --wait-children --no-convert --new-console --dll 'iexplore.exe=n' '--' '/tmp/installers/exe/IE8-WindowsXP-x86-ENU.exe' '/passive' '/update-no' '/forcerestart'
...
${INSTALLDIR}/bin/wine --bottle "${BOTTLE}" --untrusted --wait-children --no-convert --new-console -- '/tmp/installers/exe/IE8-WindowsXP-KB2936068-x86-ENU.exe' '/quiet /forcerestart'
status=$?
case $status in
  0|194) ;;
